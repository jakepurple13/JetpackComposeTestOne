...
import com.programmersbox.jetpackcomposetestone.GenericData
import com.programmersbox.jetpackcomposetestone.GenericInfo
import com.programmersbox.jetpackcomposetestone.GenericInformation
...
) : GenericInformation(name, source.url, image, description, genres, source.sources), GenericUi by AnimeUi {

    override fun rowData(): List<RowData> =
        episodes.map { RowData(it.name, it.url) { source.sources.getVideoLink(it).map { it.link.orEmpty() } } }
}
...
interface ShowApiService : GenericInfo {
    val baseUrl: String
    override val nameToUse: String get() = ""
...
enum class Sources(private val api: ShowApi) : ShowApiService by api {
    ...
    override val nameToUse: String get() = this.name
